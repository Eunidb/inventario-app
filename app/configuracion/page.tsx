...
export default function ConfiguracionPage() {
  ...
  const [modalCat, setModalCat] = useState<{ open: boolean; item: any | null }>(
  ...
  const loadData = async () => {
    setLoading(true);
    const [{ data: cats }, { data: deptos }, { data: users }] =
      await Promise.all([
  ...
  const TABS = [
    {
      id: "categorias" as Tab,
      label: "Categorías",
      Icon: Tag,
      count: categorias.length,
    },
    {
      id: "departamentos" as Tab,
      label: "Departamentos",
      Icon: Building2,
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200 shadow-xl overflow-hidden w-full">
            ...
            <div className="px-4 sm:px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center gap-4">
              ...
              <button
                onClick={() => {
                  if (tab === "categorias")
                    setModalCat({ open: true, item: null });
                  if (tab === "departamentos")
                    setModalDept({ open: true, item: null });
              ...
              <div className="divide-y divide-slate-100">
                ...
                {tab === "categorias" &&
                  categorias.map((cat) => (
                    <div
                      key={cat.id}
                      className="flex items-center justify-between px-4 sm:px-6 py-4 hover:bg-slate-50 transition-colors group gap-3"
                    >
                      ...
                      <div className="flex gap-1 shrink-0 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => setModalCat({ open: true, item: cat })}
                ...
                {tab === "categorias" && categorias.length === 0 && (
                  <EmptyState label="categorías" />
                )}
                  ...
                  departamentos.map((dept) => (
                    ...
                    >
                      ...
                      <div className="flex gap-1 shrink-0 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() =>
                            setModalDept({ open: true, item: dept })
                          }
                          className="p-2 rounded-lg text-slate-400 hover:text-[#014ba0] hover:bg-[#014ba0]/5 transition-all"
                        >
                          <Pencil size={15} />
                ...
                {tab === "departamentos" && departamentos.length === 0 && (
                  <EmptyState label="departamentos" />
                )}
                  ...
                  usuarios.map((u) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span
                          className={`px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-xl text-[9px] sm:text-[10px] font-bold border shrink-0 ${ROL_CLS[u.rol] ?? ROL_CLS.usuario}`}
                        >
                          {u.rol}
                        </span>
...
function ModalNuevoUsuario({
      ...
      <div className="space-y-4">
        ...
        {success && (
          <div className="flex items-center gap-2 p-3 bg-emerald-50 border-l-4 border-emerald-500 rounded-r-xl text-xs text-emerald-700 font-medium">
            <CheckCircle size={15} className="shrink-0" /> Usuario creado
            correctamente.
          </div>
        )}
  ...
  <div className="grid grid-cols-3 gap-1.5">
    {[
      ...
      >
        <r.Icon size={16} className="mx-auto mb-1" />
        {r.label}
      </button>
    ))}
  </div>
...
function ModalDepartamento({
  ...
  return (
    <ModalShell
      title={item ? "Editar Departamento" : "Nuevo Departamento"}
      Icon={Building2}
      onClose={onClose}
    >
      <div className="space-y-4">
