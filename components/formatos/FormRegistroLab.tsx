  ...
  const handleSave = async () => {
    ...
    await supabase.from("registros_formato").update({
      datos_json: {
        ...
        areaLab, tipoMant, descripcionServicio, parametros,
        resultado, observaciones, ejecuto, reviso, libero,
      },
      ...
      <div className="relative flex flex-col w-full max-w-3xl bg-white h-[92vh] sm:h-auto sm:max-h-[90vh] rounded-t-3xl sm:rounded-2xl shadow-2xl border border-slate-100 overflow-hidden text-slate-800">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-white shrink-0">
          <div className="flex-1 min-w-0">
            <FormHeader
              titulo="Registro de Laboratorio"
              subtitulo="Formato 5 · Equipos de Laboratorio"
              Icon={FlaskConical}
              color="rose"
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-5 bg-slate-50/50">
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field label="Área Lab."><input value={areaLab}    onChange={e => setAreaLab(e.target.value)}       className={inputCls} /></Field>
              ...
              <Field label="Tipo Mantenimiento">
                <select value={tipoMant} onChange={e => setTipoMant(e.target.value)} className={inputCls}>
                  <option>Preventivo</option><option>Correctivo</option><option>Calibración</option>
                </select>
              </Field>
            </div>
            <Field label="Descripción del servicio">
              <textarea value={descripcionServicio} onChange={e => setDescripcionServicio(e.target.value)} rows={2} className={`${inputCls} resize-none`} />
            </Field>
          </div>
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm space-y-3">
            <p className="text-[10px] font-bold text-slate-400 uppercase">Resultado del Servicio</p>
            <div className="flex gap-2 flex-wrap">
              {["Aprobado", "Condicional", "Rechazado"].map(r => (
                <button
                  key={r}
                  type="button"
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm space-y-4">
            <p className="text-[10px] font-bold text-slate-400 uppercase">Observaciones y Firmas</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <Field label="Ejecutó"><input value={ejecuto} onChange={e => setEjecuto(e.target.value)} className={inputCls} /></Field>
              <Field label="Revisó"><input value={reviso}   onChange={e => setReviso(e.target.value)}   className={inputCls} /></Field>
              <Field label="Liberó"><input value={libero}   onChange={e => setLibero(e.target.value)}   className={inputCls} /></Field>
            </div>
          </div>
